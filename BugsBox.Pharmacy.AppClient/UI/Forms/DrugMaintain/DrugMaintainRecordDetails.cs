...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.DrugMaintain
{
    public partial class DrugMaintainRecordDetails : BaseFunctionForm
    {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("提示：提交前请认真检查，一旦提交后，疑问医疗器械将进入质量复查流程，不可以修改，养护数量，需要保存养护记录吗？", "提示", MessageBoxButtons.OKCancel) != DialogResult.OK) return;
            ...
            if (this.PharmacyDatabaseService.SaveDrugMaintainDetailAndUndeterminate(list.ToArray(), out msg))
            {
                ...
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "保存医疗器械养护状态成功");
            ...
            {
                MessageBox.Show(msg);
                this.PharmacyDatabaseService.WriteLog(AppClientContext.CurrentUser.Id, "保存医疗器械养护状态失败！");

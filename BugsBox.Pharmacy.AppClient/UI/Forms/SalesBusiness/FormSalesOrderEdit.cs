...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.SalesBusiness
{
    public partial class FormSalesOrderEdit : BaseFunctionForm
    {
        ...
        public FormSalesOrderEdit()
        {
            ...
            GoodsType = GoodsTypeClass.医疗器械;
            salesDrugType = SalesDrugType.Instrument;
        }
        ...
        private void InitControl()
        {
            ...
            if (role == null)
            {
                throw new RoleNotExistsException(_salerRoleName + "不存在，请添加");
            }
            List<RoleWithUser> RoleWList = PharmacyDatabaseService.AllRoleWithUsers(out msg).Where(r => r.RoleId.Equals(role.Id)).ToList();
        ...
        private void tsbtnPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (_salesOrder != null)
                {
                    ...
                    {
                        Checker = FirstChecker + " " + SecondChecker;
                    }
                    ds.ExtendedProperties.Add("Checker", Checker);
                    string InventoryKeeper = BugsBoxApplication.Instance.InventoryKeeper;
        ...
        private void SetGridColumnVisible()
        {
            ...
            dgvDrugDetailList.Columns["商品名称"].DisplayIndex = 2;
            dgvDrugDetailList.Columns["规格"].DisplayIndex = 3;
            dgvDrugDetailList.Columns["批号"].DisplayIndex = 4;
            dgvDrugDetailList.Columns["生产厂商"].DisplayIndex = 5;
            dgvDrugDetailList.Columns["单位"].DisplayIndex = 6;
            dgvDrugDetailList.Columns["采购单价"].DisplayIndex = 7;

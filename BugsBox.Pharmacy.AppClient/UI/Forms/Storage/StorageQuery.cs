...
namespace BugsBox.Pharmacy.AppClient.UI.Forms.Storage
{
    public partial class StorageQuery : BaseFunctionForm
    {
        ...
        public StorageQuery(string type)
        {
            ...
            if (!this.Authorize(ModuleKeys.YKSQ)) return;
            ToolStripMenuItem tsi = new ToolStripMenuItem("移至－>");
            var warehousezoneArr = this.comboBox1.DataSource as List<WarehouseZone>;
        ...
        private void Search()
        {
            try
            {
                ...
                string msg = String.Empty;
                storage = PharmacyDatabaseService.StorageQuery(out msg, this.textBox1.Text.Trim(), this.textBox2.Text.Trim(), this.textBox3.Text.Trim(), warehouseZonesIds, this.pagerControl1.PageIndex, this.pagerControl1.PageSize, new object[] { }).ToList();
        ...
        private void InstSearch()
        {
            InstInventoryQueryModel q = new InstInventoryQueryModel
            {
                BatchNumber = this.textBox3.Text.Trim(),
                Keyword = this.textBox1.Text.Trim(),
                FactoryName = this.textBox2.Text.Trim(),
                DrugCategoryCode = this.comboBox2.SelectedItem == "全部" ? "" : this.comboBox2.SelectedItem.ToString(),
                WarehouseZoneId=(Guid)this.comboBox1.SelectedValue,
                PageSize=this.pagerControl1.PageSize,
                PageIndex=this.pagerControl1.PageIndex-1
            };

            var re = this.PharmacyDatabaseService.InstStorageQuery(q, out msg).ToList();
            this.dataGridView1.DataSource = re;
            this.pagerControl1.RecordCount = re.Count>0?re.First().RecordCount:0;
            label5.Text = string.Format("总数：{0}",re.Sum(r=>r.CanSaleNum));
            label6.Text = string.Format("总价值:{0}元", re.Sum(r=>r.PriceCount)); ;
        }

        private void BindComboBox()
        {
            string msg = string.Empty;
            var listWarehouseZone = PharmacyDatabaseService.AllWarehouseZones(out msg).ToList();
            listWarehouseZone.Insert(0, new WarehouseZone
            {
                Id = Guid.Empty,
                Name = "全部"
            });
            ...
            this.comboBox2.Items.Add("一类");
            this.comboBox2.Items.Add("二类");
            this.comboBox2.Items.Add("三类");
        ...
        {
            var m = this.dataGridView1.CurrentRow.DataBoundItem as Business.Models.InstInventeryModel;
            if (m.PurchaseInInventoryDetailId == Guid.Empty)
            {
                MessageBox.Show("前期库存，无入库信息！请到老系统中查询入库相关信息！");
                return;
            }

            Common.Form_HistoryPurchase frm = new Common.Form_HistoryPurchase(m.PurchaseInInventoryDetailId);
            frm.ShowDialog();
        ...
        {
            var m = this.dataGridView1.CurrentRow.DataBoundItem as Business.Models.InstInventeryModel;
            Common.Form_HistoryPurchase frm = new Common.Form_HistoryPurchase(m.DrugInfoId, 1);
            frm.ShowDialog();
            this.dataGridView1.Focus();
        ...
        private void dataGridView1_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                Business.Models.InstInventeryModel im = this.dataGridView1.CurrentRow.DataBoundItem as Business.Models.InstInventeryModel;

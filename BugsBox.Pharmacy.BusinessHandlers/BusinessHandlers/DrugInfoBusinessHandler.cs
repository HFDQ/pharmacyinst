    ...
    partial class DrugInfoBusinessHandler
    {
        ...
        public IEnumerable<DrugInfo> GetDrugInfoByPurchaseUnit(Guid purchaseUnitGuid)
        {
            try
            {
                ...
                if (purchaseUnitGuid != Guid.Empty)
                {
                    ...
                    List<string> businessScopes = BusinessHandlerFactory.PurchaseUnitBusinessHandler.GetBusinessScopeCodesByPurchaseUnit(purchaseUnit);
                    var result = this.Fetch(p => p.Valid && p.Enabled);//庄子不控
                    result = result.Where(p => businessScopes.Contains(p.StandardCode));
                    return result;
                }
                else
                {
        ...
        public IEnumerable<DrugInfo> GetDrugInfoBySupplyUnit(Guid supplyUnitGuid)
        {
            try
            {
                ...
                var result = this.Fetch(p => p.Valid && p.Enabled && businessScope.Contains(p.StandardCode));

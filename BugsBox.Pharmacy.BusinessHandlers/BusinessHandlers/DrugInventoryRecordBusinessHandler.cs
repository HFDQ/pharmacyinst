...
namespace BugsBox.Pharmacy.BusinessHandlers
{
    partial class DrugInventoryRecordBusinessHandler
    {
        ...
        /// 医疗器械可用库存查询
        /// </summary>
        /// <param name="q">query模型</param>
        /// <returns></returns>
        public IEnumerable<BugsBox.Pharmacy.Business.Models.InstInventeryModel> InstStorageQuery(BugsBox.Pharmacy.Business.Models.InstInventoryQueryModel q)
        {
            try
            {
                var c = this.Queryable.Where(r=>r.CanSaleNum>0);
                #region 查询条件
                if (!string.IsNullOrEmpty(q.Keyword))
                {
                    if (q.IsAccurate)
                    {
                        c = c.Where(r => r.DrugInfo.ProductGeneralName.Contains(q.Keyword) || r.DrugInfo.Pinyin.Contains(q.Keyword.ToUpper()) || r.DrugInfo.StandardCode.Contains(q.Keyword));//品名、拼音、分类码
                    }
                    else
                    {
                        c = c.Where(r => r.DrugInfo.ProductGeneralName == (q.Keyword) || r.DrugInfo.Pinyin == (q.Keyword.ToUpper()) || r.DrugInfo.StandardCode == (q.Keyword));//品名、拼音、分类码
                    }
                }
                if (q.WarehouseZoneId != Guid.Empty)
                {
                    c = c.Where(r => r.WarehouseZoneId == q.WarehouseZoneId);
                }
                if (!string.IsNullOrEmpty(q.DrugCategoryCode))
                {
                    if (q.IsAccurate)
                    {
                        c = c.Where(r => r.DrugInfo.DrugCategoryCode.Contains(q.DrugCategoryCode));
                    }
                    else
                    {
                        c = c.Where(r => r.DrugInfo.DrugCategoryCode == (q.DrugCategoryCode));
                    }
                }
                if (!string.IsNullOrEmpty(q.BatchNumber))
                {
                    if (q.IsAccurate)
                    {
                        c = c.Where(r => r.BatchNumber.Contains(q.BatchNumber));
                    }
                    else
                    {
                        c = c.Where(r => r.BatchNumber == (q.BatchNumber));
                    }
                }
                #endregion
                int recordCount = c.Count();
                #region Linq联表
                var re = from i in c
                         join pinv in this.RepositoryProvider.Db.PurchaseInInventeryOrderDetails
                         on i.PurchaseInInventeryOrderDetailId equals pinv.Id
                         select new BugsBox.Pharmacy.Business.Models.InstInventeryModel
                         {
                             BatchNumber = i.BatchNumber,
                             isValid = i.Valid,
                             CanSaleNum = i.CanSaleNum,
                             ...
                             DrugCategoryCode = i.DrugInfo.DrugCategoryCode,
                             DrugInfoId = i.DrugInfoId,
                             FactoryName = i.DrugInfo.FactoryName,
                             InInventoryCount = i.InInventoryCount,
                             InstEntProductLiscencePermitNumber = i.DrugInfo.InstEntProductLiscencePermitNumber,
                             InventoryID = i.Id,
                             IsOutDate = i.IsOutValidDate ? "过期" : "未过期",
                             LicensePermissionNumber = i.DrugInfo.LicensePermissionNumber,
                             Origin = i.DrugInfo.Origin,
                             OutValidDate = i.OutValidDate,
                             ProductGeneralName = i.DrugInfo.ProductGeneralName,
                             PruductDate = i.PruductDate,
                             PurchasePrice = i.PurchasePricce,
                             StandardCode = i.DrugInfo.StandardCode,
                             WarehouseZoneName = i.WarehouseZone.Name,
                             SupplyUnitName = pinv == null ? "前期库存,无入库信息" : pinv.PurchaseInInventeryOrder.PurchaseOrder.SupplyUnit.Name,
                             PurchaseOrderDocumentNumber = pinv == null ? "前期库存,无入库信息" : pinv.PurchaseInInventeryOrder.PurchaseOrder.DocumentNumber,
                             RecordCount = recordCount,
                              PurchaseInInventoryDetailId=i.PurchaseInInventeryOrderDetailId
                         };                 
                #endregion
                return re.OrderBy(r=>r.ProductGeneralName).Skip(q.PageIndex).Take(q.PageSize);
            }
            finally
            {
                this.Dispose();

	...
	{  
	 ...
	 [FaultContract(typeof(ServiceExceptionDetail))]
       ...
       #region 新增直调销售表（事务）
        ...
        [OperationContract]
        [FaultContract(typeof(ServiceExceptionDetail))]
        int GetSupplyUnitCount(out string message);
        #endregion

        #region 调用保存GSP证书和代码
        [OperationContract]
        [FaultContract(typeof(ServiceExceptionDetail))]
        bool EditGSPLiscence(GSPLicense m, out string message);
        #endregion

        #region 调用医疗器械可用库存查询
        [OperationContract]
        [FaultContract(typeof(ServiceExceptionDetail))]
        IEnumerable<BugsBox.Pharmacy.Business.Models.InstInventeryModel> InstStorageQuery(BugsBox.Pharmacy.Business.Models.InstInventoryQueryModel q, out string message);
        #endregion

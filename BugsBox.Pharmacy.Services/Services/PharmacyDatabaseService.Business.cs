...
{ 
    ...
    partial class PharmacyService
    {
        ...
        public int GetSupplyUnitCount(out string message)
        {
            ...
            try
            {
                return HandlerFactory.SupplyUnitBusinessHandler.GetSupplyUnitCount();
            }
            catch (Exception ex)
            {
                message = "调用供货单位总数失败";
        ...
        public bool EditGSPLiscence(GSPLicense m, out string message)
        {
            ...
            try
            {
                return HandlerFactory.GSPLicenseBusinessHandler.EditGSPLiscence(m);
            }
            catch (Exception ex)
            {
                message = "调用保存GSP证书和代码失败";
                return this.HandleException<bool>(message, ex);
            }
            finally
            {
        ...
        #region 医疗器械可用库存查询
        public IEnumerable<BugsBox.Pharmacy.Business.Models.InstInventeryModel> InstStorageQuery(BugsBox.Pharmacy.Business.Models.InstInventoryQueryModel q, out string message)
        {
            message = string.Empty;
            try
            {
                return HandlerFactory.DrugInventoryRecordBusinessHandler.InstStorageQuery(q);
            }
            catch (Exception ex)
            {
                message = "调用医疗器械可用库存查询失败";
                return this.HandleException<IEnumerable<BugsBox.Pharmacy.Business.Models.InstInventeryModel>>(message, ex);
            }
            finally
            {
                this.Dispose();
